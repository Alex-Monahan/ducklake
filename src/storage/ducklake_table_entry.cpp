...
static bool PartitionFieldsMatch(optional_ptr<DuckLakePartition> current_partition,
	...
	for (idx_t i = 0; i < current_partition->fields.size(); i++) {
		auto &lhs = current_partition->fields[i];
		auto &rhs = requested_partition.fields[i];
		if (lhs.field_id != rhs.field_id) {
			return false;
		}
		if (lhs.transform.type != rhs.transform.type) {
			return false;
		}
		if (lhs.transform.bucket_count != rhs.transform.bucket_count) {
			return false;
		}
		if (lhs.partition_key_index != rhs.partition_key_index) {
			return false;
		}
	}
	return true;
...
DuckLakeTableEntry::BuildPartitionData(DuckLakeTransaction &transaction, const ColumnList &columns,
	...
	for (idx_t expr_idx = 0; expr_idx < partition_keys.size(); expr_idx++) {
		...
		for (auto &existing : partition_data->fields) {
			if (existing.field_id == partition_field.field_id &&
			    existing.transform.type == partition_field.transform.type &&
			    existing.transform.bucket_count == partition_field.transform.bucket_count) {

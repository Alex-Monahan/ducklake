...
string DuckLakeMetadataManager::FormatCatalogId(idx_t id) {
	return to_string(id);
}

string DuckLakeMetadataManager::FormatFileId(idx_t id) {
	return to_string(id);
}

string DuckLakeMetadataManager::FormatRowId(TableIndex table_id, optional_idx row_id_start,
...
string DuckLakeMetadataManager::WriteNewSchemas(const vector<DuckLakeSchemaInfo> &new_schemas) {
	...
	for (auto &new_schema : new_schemas) {
		...
		schema_insert_sql += StringUtil::Format(
		    "(%s, '%s', {SNAPSHOT_ID}, NULL, %s, %s, %s)", FormatCatalogId(new_schema.id.index), new_schema.uuid,
		    SQLString(new_schema.name), SQLString(path.path), path.path_is_relative ? "true" : "false");
	}
...
static void ColumnToSQLRecursive(const DuckLakeColumnInfo &column, const string &formatted_table_id,
                                 optional_idx parent, string &result) {
	if (!result.empty()) {
		result += ",";
	}
	...
	result += StringUtil::Format("(%d, {SNAPSHOT_ID}, NULL, %s, %d, %s, %s, %s, %s, %s, %s, %s, %s)", column_id,
	                             formatted_table_id, column_order, SQLString(column.name), SQLString(column.type),
	                             initial_default_val, default_val, column.nulls_allowed ? "true" : "false", parent_idx,
	                             default_val_type, default_val_system);
	for (auto &child : column.children) {
		ColumnToSQLRecursive(child, formatted_table_id, column_id, result);
	}
}
...
string DuckLakeMetadataManager::WriteNewTables(DuckLakeSnapshot commit_snapshot,
	...
	for (auto &table : new_tables) {
		...
		table_insert_sql +=
		    StringUtil::Format("(%s, '%s', {SNAPSHOT_ID}, NULL, %s, %s, %s, %s)", FormatCatalogId(table.id.index),
		                       table.uuid, FormatCatalogId(table.schema_id.index), SQLString(table.name),
		                       SQLString(path.path), path.path_is_relative ? "true" : "false");
		for (auto &column : table.columns) {
			ColumnToSQLRecursive(column, FormatCatalogId(table.id.index), optional_idx(), column_insert_sql);
		}
	}
	string batch_query;
...
string DuckLakeMetadataManager::GetInlinedTableName(const DuckLakeTableInfo &table, const DuckLakeSnapshot &snapshot) {
...
string DuckLakeMetadataManager::GetInlinedTableQueries(DuckLakeSnapshot commit_snapshot, const DuckLakeTableInfo &table,
	...
	inlined_tables += StringUtil::Format("(%s, %s, %s)", FormatCatalogId(table.id.index), SQLString(inlined_table_name),
	                                     FormatSchemaVersion(commit_snapshot));
...
INSERT INTO {METADATA_CATALOG}.ducklake_macro values(%s,%s,'%s',{SNAPSHOT_ID}, NULL);
		                                 ...
		                                 FormatCatalogId(macro.schema_id.index), FormatCatalogId(macro.macro_id.index),
		                                 macro.macro_name);
...
INSERT INTO {METADATA_CATALOG}.ducklake_macro_impl values(%s,%llu,'%s','%s','%s');
			                       ...
			                       FormatCatalogId(macro.macro_id.index), impl_id, impl.dialect, impl.sql, impl.type);
...
INSERT INTO {METADATA_CATALOG}.ducklake_macro_parameters values(%s,%llu,%llu,'%s','%s','%s', '%s');
				                       ...
				                       FormatCatalogId(macro.macro_id.index), impl_id, param_id, param.parameter_name,
...
string DuckLakeMetadataManager::WriteNewColumns(const vector<DuckLakeNewColumn> &new_columns) {
	...
	for (auto &new_col : new_columns) {
		ColumnToSQLRecursive(new_col.column_info, FormatCatalogId(new_col.table_id.index), new_col.parent_idx,
		                     column_insert_sql);
	}
...
string DuckLakeMetadataManager::WriteNewViews(const vector<DuckLakeViewInfo> &new_views) {
	...
	for (auto &view : new_views) {
		...
		view_insert_sql += StringUtil::Format(
		    "(%s, '%s', {SNAPSHOT_ID}, NULL, %s, %s, %s, %s, %s)", FormatCatalogId(view.id.index), view.uuid,
		    FormatCatalogId(view.schema_id.index), SQLString(view.name), SQLString(view.dialect), SQLString(view.sql),
    ...
    WHERE table_id=%d
		...
		for (auto &chunk : entry.data->data->Chunks()) {
			for (idx_t r = 0; r < chunk.size(); r++) {
				...
				values += "(";
				if (entry.row_id_offset.IsValid()) {
					// SP mode: use placeholder-based row IDs
					auto per_row_offset = optional_idx(entry.row_id_offset.GetIndex() + row_idx);
					values += FormatRowId(entry.table_id, optional_idx(entry.row_id_start + row_idx), per_row_offset);
				} else {
					values += to_string(entry.row_id_start + row_idx);
				}
...
string DuckLakeMetadataManager::WriteNewDataFiles(const vector<DuckLakeFileInfo> &new_files,
	...
	for (auto &file : new_files) {
		...
		auto row_id =
		    file.row_id_start.IsValid() ? FormatRowId(file.table_id, file.row_id_start, file.row_id_offset) : "NULL";
		...
		auto data_file_index = FormatFileId(file.id.index);
		auto table_id = FormatCatalogId(file.table_id.index);
		auto encryption_key =
		...
		data_file_insert_query += StringUtil::Format(
		    "(%s, %s, %s, NULL, NULL, %s, %s, 'parquet', %d, %d, %s, %s, %s, %s, %s, %s)", data_file_index, table_id,
		...
		for (auto &column_stats : file.column_stats) {
			...
			column_stats_insert_query += StringUtil::Format(
			    "(%s, %s, %d, %s, %s, %s, %s, %s, %s, %s)", data_file_index, table_id, column_id,
			...
			for (auto &variant_stats : column_stats.variant_stats) {
				...
				variant_stats_insert_query += StringUtil::Format(
				    "(%s, %s, %d, %s, %s, %s, %s, %s, %s, %s, %s, %s)", data_file_index, table_id, column_id,
		...
		for (auto &part_val : file.partition_values) {
			...
			partition_insert_query +=
			    StringUtil::Format("(%s, %s, %d, %s)", data_file_index, table_id, part_val.partition_column_idx,
...
string DuckLakeMetadataManager::WriteNewDeleteFiles(const vector<DuckLakeDeleteFileInfo> &new_files,
	...
	for (auto &file : new_files) {
		...
		auto delete_file_index = FormatFileId(file.id.index);
		auto table_id = FormatCatalogId(file.table_id.index);
		auto data_file_index = FormatFileId(file.data_file_id.index);
		auto encryption_key =
		    file.encryption_key.empty() ? "NULL" : "'" + Blob::ToBase64(string_t(file.encryption_key)) + "'";
		...
		delete_file_insert_query += StringUtil::Format(
		    "(%s, %s, %s, NULL,  %s, %s, %s, 'parquet', %d, %d, %d, %s, %s)", delete_file_index, table_id,
...
string DuckLakeMetadataManager::WriteNewColumnMappings(const vector<DuckLakeColumnMappingInfo> &new_column_mappings) {
	...
	for (auto &column_mapping : new_column_mappings) {
		...
		column_mapping_insert_query +=
		    StringUtil::Format("(%s, %s, %s)", FormatCatalogId(column_mapping.mapping_id.index),
		                       FormatCatalogId(column_mapping.table_id.index), SQLString(column_mapping.map_type));
		for (auto &name_map_column : column_mapping.map_columns) {
			...
			name_map_insert_query +=
			    StringUtil::Format("(%s, %d, %s, %d, %s, %s)", FormatCatalogId(column_mapping.mapping_id.index),
...
string DuckLakeMetadataManager::WriteNewPartitionKeys(DuckLakeSnapshot commit_snapshot,
	...
	for (auto &new_partition : new_partition_map) {
		...
		new_partition_values += StringUtil::Format(R"((%s, %s, {SNAPSHOT_ID}, NULL))", FormatCatalogId(partition_id),
		                                           FormatCatalogId(new_partition.second.table_id.index));
		for (auto &field : new_partition.second.fields) {
			if (!insert_partition_cols.empty()) {
				insert_partition_cols += ", ";
			}
			insert_partition_cols +=
			    StringUtil::Format("(%s, %s, %d, %d, %s)", FormatCatalogId(partition_id),
			                       FormatCatalogId(new_partition.second.table_id.index), field.partition_key_index,
...
string DuckLakeMetadataManager::WriteNewSortKeys(DuckLakeSnapshot commit_snapshot,
	...
	for (auto &new_sort : new_sort_map) {
		...
		new_sort_values += StringUtil::Format(R"((%s, %s, {SNAPSHOT_ID}, NULL))", FormatCatalogId(sort_id),
		                                      FormatCatalogId(new_sort.second.table_id.index));
		...
		for (auto &field : new_sort.second.fields) {
			...
			new_sort_expressions += StringUtil::Format(
			    "(%s, %s, %d, %s, %s, %s, %s)", FormatCatalogId(sort_id),
			    FormatCatalogId(new_sort.second.table_id.index), field.sort_key_index, SQLString(field.expression),
...
string DuckLakeMetadataManager::UpdateGlobalTableStats(const DuckLakeGlobalStatsInfo &stats) {
	...
	if (!stats.initialized) {
		...
		batch_query += StringUtil::Format(
		    "INSERT INTO {METADATA_CATALOG}.ducklake_table_stats VALUES (%s, %d, %d, %d);",
		    FormatCatalogId(stats.table_id.index), stats.record_count, stats.next_row_id, stats.table_size_bytes);
	...
	for (idx_t i = 0; i < compactions.size(); ++i) {
		...
		batch_query += StringUtil::Format(R"(
			...
			WHERE data_file_id = %s;
		                                  ...
		                                  table_idx_last_snapshot[compaction.table_index.index],
		                                  FormatFileId(compaction.new_id.index));
	}
	return batch_query;
}
...
string DuckLakeMetadataManager::InsertNewSchema(const DuckLakeSnapshot &snapshot, const set<TableIndex> &table_ids) {
	...
	for (auto &table_id : table_ids) {
		result += StringUtil::Format(
		    R"(INSERT INTO {METADATA_CATALOG}.ducklake_schema_versions VALUES ({SNAPSHOT_ID},{SCHEMA_VERSION},%d);)",
		    table_id.index);
	}
	return result;
}
